...
// POST — admin creates invitation for an existing user
export async function POST(req) {
  const session = await auth();
  if (!session?.user?.isAdmin) return NextResponse.json({ error: "Forbidden" }, { status: 403 });

  const { userId, email, expiresInDays = 14 } = await req.json();
  if (!userId || !email) return NextResponse.json({ error: "userId and email required" }, { status: 400 });

  // Delete any existing unused invitation for this user first
